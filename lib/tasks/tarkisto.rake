namespace :tarkisto do
  ...
  task :fetch_courses => :environment do
    ...
    courses.each do |course|
      next if Course.find_by_code(course.code)
      level = nil
      if course.basic?
        level = Level.basic_studies
      elsif course.intermediate?
        level = Level.intermediate_studies
      elsif course.advanced?
        level = Level.advanced_studies
      else
        level = Level.other_studies
      end
  ...
  namespace :example_data do
    ...
    task :themes => :environment do
      algorithm_theme = Theme.find_or_create_by_name_fi 'Algoritmit ja ohjausrakenteet'
      variable_theme = Theme.find_or_create_by_name_fi 'Muuttujat ja tyypit'
      subroutines_theme = Theme.find_or_create_by_name_fi 'Aliohjelmat'
      class_theme = Theme.find_or_create_by_name_fi 'Luokat'
      objects_theme = Theme.find_or_create_by_name_fi 'Oliot ja kapselointi'

      Course.find_by_code(581325).themes << algorithm_theme rescue nil
      Course.find_by_code(581325).themes << variable_theme rescue nil
      Course.find_by_code(581325).themes << subroutines_theme rescue nil
      Course.find_by_code(581325).themes << class_theme rescue nil
      Course.find_by_code(581325).themes << objects_theme rescue nil
    end

  end

  task :example_data => :environment do
    algorithm_theme = Theme.find_by_name_fi 'Algoritmit ja ohjausrakenteet'
    variable_theme = Theme.find_by_name_fi 'Muuttujat ja tyypit'
    subroutines_theme = Theme.find_by_name_fi 'Aliohjelmat'
    class_theme = Theme.find_by_name_fi 'Luokat'
    objects_theme = Theme.find_by_name_fi 'Oliot ja kapselointi'

    alg = algorithm_theme.course_themes.find_by_course_id Course.find_by_code(581325).id
    var = variable_theme.course_themes.find_by_course_id Course.find_by_code(581325).id
    sub = subroutines_theme.course_themes.find_by_course_id Course.find_by_code(581325).id
    classes = class_theme.course_themes.find_by_course_id Course.find_by_code(581325).id
    objects = objects_theme.course_themes.find_by_course_id Course.find_by_code(581325).id

    Course.find_by_code(581325).exams.create :lecturer => 2, :type_id => Type.separate_exams.id, :date => Date.new(2010, 6, 1), :maximum_points => 60
    Course.find_by_code(581325).exams.only_public.create :lecturer => 2, :type_id => Type.renewal_exams.id, :date => Date.new(2010, 8, 30), :maximum_points => 50

    Course.find_by_code(582206).exams.create :lecturer => 3, :type_id => Type.separate_exams.id, :date => Date.new(2010, 6, 4), :maximum_points => 60
    Course.find_by_code(582206).exams.create :lecturer => 3, :type_id => Type.course_exams.id, :date => Date.new(2009, 10 , 22), :maximum_points => 60
    Course.find_by_code(582206).exams.create :lecturer => 3, :type_id => Type.renewal_exams.id, :date => Date.new(2010, 1, 22), :maximum_points => 60

    Course.find_by_code(582313).exams.create :lecturer => 4, :type_id => Type.course_exams.id, :date => Date.new(2009, 10, 21), :maximum_points => 60

    kesa = Course.find_by_code(581325).exams.only_public.create :lecturer => 2, :type_id => Type.course_exams.id, :date => Date.new(2010, 6, 12), :maximum_points => 50, :language => 'fi'

    unless kesa.new_record?
...
Arvattavien lukujen hakeminen on tehokkuussyistä ohjelmoitava binäärihakua käyttäen. &, :name => %&Toteuta seuraava arvauspeli vuorovaikutteisena ohjelmana:&, :points => 17
    ...
    ohpes2009 = Course.find_by_code(581325).exams.only_public.create :lecturer => 2, :type_id => Type.course_exams.id, :date => Date.new(2009, 10, 21), :maximum_points => 50, :language => 'fi'

    unless ohpes2009.new_record?
...
Havainnollista Saatio-luokan käyttöä pienellä pääohjelmalla. Tämän ohjelman ei tarvitse eikä ole syytä lukea mitään. Älä turhaan yritä etsiä Saatio-luokalle vastineita todellisuudesta! Ohjelmoi vain niin kuin "API" käskee. &, :description => "", :points => 16
    ...
    ohpek2008 = Course.find_by_code(581325).exams.only_public.create :lecturer => 2, :type_id => Type.course_exams.id, :date => Date.new(2008, 6, 14), :maximum_points => 50, :language => 'fi'

    unless ohpek2008.new_record?
...
Laadi myös pääohjelma, joka esittelee ja havainnollistaa VirvoitusjuomaAutomaatti-olioiden käyttöä.&, :description => "", :points => 16
    ...
    ohpek2007 = Course.find_by_code(581325).exams.only_public.create :lecturer => 2, :type_id => Type.course_exams.id, :date => Date.new(2007, 6, 9), :maximum_points => 50, :language => 'fi'

    unless ohpek2007.new_record?
...
Varusta luokka metodilla toString(), jonka ansiosta olion arvo saadaan tulostettua muodossa tunnit:minuutit:sekunnit, siis esimerkiksi "14:7:53". &, :description => "", :points => 16
      ...
      ohpek2007.questions.last.course_theme_questions.create :course_theme => var
      ohpek2007.questions.last.course_theme_questions.create :course_theme => classes
      ohpek2007.questions.last.course_theme_questions.create :course_theme => sub
    ...
    ohmas08 = Course.find_by_code(582104).exams.only_public.create :lecturer => 5, :type_id => Type.course_exams.id, :date => Date.new(2009, 12, 16), :maximum_points => 28, :language => 'fi'

    unless ohmas08.new_record?
